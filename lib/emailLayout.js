 ...
 * not a black "mourning frame". Assets are served from the production
 * domain (elmeny.hu).
 */
...
// Pre-composed orange header banner (white logo + rocket doodle) — one
// flat image renders identically in every client; the cell's bgcolor is
// the fallback when images are blocked.
const HEADER = 'https://elmeny.hu/assets/email/header.png'
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="max-width:600px;width:100%;background:#ffffff;border-radius:22px;overflow:hidden;border:1px solid ${C.border};">
  <tr><td align="center" bgcolor="${C.red}" style="background:${C.red};font-size:0;line-height:0;">
    <img src="${HEADER}" width="600" alt="Élménypont" style="display:block;width:100%;max-width:600px;height:auto;border:0;outline:none;text-decoration:none;">
  </td></tr>
  <tr><td align="center" bgcolor="${C.red}" style="background:${C.red};padding:0 24px 18px;font-family:Arial,Helvetica,sans-serif;font-size:11px;letter-spacing:.2em;text-transform:uppercase;color:rgba(255,255,255,.9);">${esc(badge)}</td></tr>
